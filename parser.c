...
unsigned int get_active_process_count(char *path_mem_dump)
{
	...
	count = 0;
	for (addr = 0; addr < s_stat.st_size; addr += PD_SIZE) {
...
static pdir_t *get_pdir(int fd, unsigned int pdir_addr)
{
	...
	assert(p_pdir != NULL);
	p_pdir->pde_ptr = mmap(NULL, PD_SIZE, PROT_READ, MAP_PRIVATE, fd, pdir_addr);
...
static void fill_valid_pde_count(pdir_t *p_pdir)
{
	...
	for (i = 0; i < NUM_PDE; i++) {
		if (p_pdir->pde_ptr[i] & PDE_FLAG_MASK == PDE_FLAG_SIG) {
			p_pdir->pde_match++;
		}
	}
...
static bool is_valid_pdir(pdir_t *p_pdir)
{
	...
	if (p_pdir->pde_match >= VALID_PDE_THRESHOLD) {
		return true;
	}
